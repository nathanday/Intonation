	...
	override func drawRect(dirtyRect: NSRect) {
		...
		func drawAxises() {
			...
			for i in 1..<8*commonFactor {
				...
				if i%8 == 4 {
					drawText(string: "\(i/8 + 1)", size: NSFont.systemFontSizeForControlSize(NSControlSize.RegularControlSize)*1.25, point: NSMakePoint(theX-6.0,20.0), color:NSColor.darkGrayColor(), textAlignment:.Center );
				}
			}
